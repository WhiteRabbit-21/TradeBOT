...
def fetch_position_oneway_sync(symbol: str) -> Optional[dict]:
    try:
...
async def fetch_position_oneway(symbol: str) -> Optional[dict]:
    return await asyncio.to_thread(fetch_position_oneway_sync, symbol)

def close_position_full_oneway_sync(base: str) -> str:
...
def set_sl_oneway_sync(base: str, sl_price: float) -> str:
    ...
    for order_type, params in candidates:
        try:
            resp = exchange.create_order(symbol, order_type, stop_side, contracts, None, params)
            new_id = resp.get("id")
            if old:
...
def set_tp_oneway_sync(base: str, tp_price: float) -> str:
    ...
    resp = exchange.create_order(symbol, "limit", close_side, contracts, tp_prec, {"reduceOnly": True})
...
def add_position_oneway_sync(base: str, add_pct: Optional[float]) -> str:
    ...
    resp = open_market_sync(symbol, pos_side, add_qty)
